...
public class Ball : MonoBehaviour
{
        ...
        switch(_positionState)
        {
            ...
            case PositionState.Moving:
            {
                if(!(_movedDistance+movingSpeed>_boardManager.holeDistance))
                {
                    ...
                    gameObject.transform.position = getPosition(_movedDistance,_cubeCoord+Utils.GetDirectionByNum(_direction)*-1);
                }
                else
                {
                    _direction = -1;
    ...
    public void StopPushing(bool isCanceled)
    {
        ...
        _cubeCoord += Utils.GetDirectionByNum(_direction);
    ...
    {
        _cubeCoord += Utils.GetDirectionByNum(direction);
        _direction = direction;
        _movedDistance = 0;
        _positionState = PositionState.Moving;
    }
